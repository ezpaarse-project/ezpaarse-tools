 ...
 * Script qui sert à injecter certaines colonnes d'un fichier de référence sur les lignes du fluxé * d'entrée ou des fichiers traités.é * La détermination de la ligne de référence à utiliser se base sur l'identité de contenu d'une
...
/* Tableau  de couple de caractères utilisés en début et fin de valeur unitaireé * notamment afin que les valeurs puissent contenir des caratères séparateur
 * ex. cuple [] pour le séparateur ,é * [Auteur, Marc],[Titre,à virgule],Flammarion,1957é */
$limites_valeurs = array(array('"','"'),array('[',']'));
/**
 * Traitement de la ligne de heaedr qui consiste à établir un lien entre nom de colonne et sa position.é * Le tableau $conv_colnum est construit lors de l'analyse le commande avec toutes les colonnes désignées.
 * @param string $ligne : chaîne (ligne d'entête ) contenant le format des lignes
 * @return boolean : vrai si OK. sinon étblit la chaîne globale $echo_err. é */
...
/**
 *
 * projete_temps : Conversion d'une chaîne tampon en temps timestampé * @param string $chaine_tampon = la chaîne
...
// ... et du fichier source nécessaireé$sources=array(); $references=array(); $fic_res="";$fic_resr="";$fic_rej="";$fic_par="";
...
if ($mod_col_referentiel=='nom') {
	...
	if ( !$ligne_header_referentiel) {
		...
		/* Homogénéïsation de la forme du tableau des colonnes particulierement utiles. */é} else {
...
if (!$est_vrai_ref){
	...
	/*
	 * ===============================================================================
	 * Lecture des fichiers référence - création d'un fichier intermédiaire a trieré	 * ===============================================================================
	 */
