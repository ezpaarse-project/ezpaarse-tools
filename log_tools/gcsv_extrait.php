 ...
 *  Par défaut, il y deux parenthéseurs (utilisé classiquement dans les logs) :
 *  la double quote("") ou le couple crochet ouvrant/fermant ([]).
 *  Pour changer les parenthéseurs, utiliser -par fichier_parentheses. Le contenu du fichier écrase les
 ...
 *       paramétres s f et fh (qui peuvent être employés sans précision de colonne)
 *
 *  ... V0.3 (2013.04.09)
...
** Exemples d'emplois :
  ...
  * faire de plusieurs fichiers à entête ezproxy, un seul fichier avec la nouvelle entete
...
./gcsv_extrait.php -src format.txt mes_log* -allcol +hd -colca login -colcip IP
  * supprimer des colonnes. L'entête est corrigee en consequence :
./gcsv_extrait.php -src format.txt mes_log* +hd -col login code  session
  * réordonner et coder des colonnes. Ne retenir que date, coder IP et login, ne retenir que le host url
   et groupes  :
...
function traite_fichier_parentheses_mess($no_mess,$param=""){
	switch ($no_mess){
		case 'FINC' :
			return ("Fichier de parenthéseur inconnu $param.\n");
		case 'FINV' :
			return ("Fichier de parenthéseur $param invalide.\n");
		default:
			if (is_array($param)) $param=print_r($param,true);
			return ("traite_fichier_parentheses_mess erreur inconnue $no_mess pour $param.\n");
	}
}
...
function message ($no,$param=''){
	...
	switch ($no){
		...
		case 'colTERR' :
			...
			return ("Critère $crit en colonne $col erronné : $mess.\n");
		case 'colTVAL' :
			$col = $param[0];$crit=$param[1];$val = $param[2];
			return ("Critère $crit sur $val en colonne $col.\n");
		case 'quoi' :
		...
		case 'Mod_nomINV' :
			return ("Colonnes designées par leur nom mais pas de ligne d'entête declarée.\n");
		case 'FicResNOP' ://
		...
		case 'final' :
			...
			return ("$cptlues lignes lues, $cptecrites lignes ecrites en $tmis s\n");
		default:
			if (is_array($param)) $param=print_r($param,true);
